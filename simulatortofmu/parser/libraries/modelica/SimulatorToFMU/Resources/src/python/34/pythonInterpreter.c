...
void pythonExchangeVariables(const char * moduleName,
	...
	/* Load Python module*/
        ...
        if (!ptrMemory->isInitialized) {
          /* PyUnicode in Python34 is PyString in Python34*/
		...
		if (nDblRea > 0){
			...
			*/
			if (nDblRea == 1){
				...
				*/
				if (PyFloat_Check(pItemDbl) || PyLong_Check(pItemDbl))
	...
	/* simulateModel("the SimulatorToFMU.Utilities.IO.Python34.Functions.Examples.TestPythonInterface");*/
	/**/
