...
///////////// THE CODE BELOW HAS BEEN AUTOGENERATED //////////////
  ...
  Modelica.Blocks.Interfaces.RealOutput {{dict_item["name"]}} (unit="{{dict_item["unit"]}}")
    "{{dict_item["description"]}}"{{dict_item["annotation"]}};
  // Edited by Christoph on 12/06/2018 //	
  Modelica.Blocks.Continuous.FirstOrder {{dict_item["name"]}}_transfer (T=Tfirstorder);
    /*"{{dict_item["description"]}}"{{dict_item["annotation"]}};*/
  // End //
  {%- endif -%}
  {% endfor %}
...
///////////// THE CODE ABOVE HAS BEEN AUTOGENERATED //////////////
  ...
  // Edited by Christoph on 12/06/2018 //
	// Simulator data
	{%- if exec_target=="python" %}
	dblOutVal = SimulatorToFMU.Python{{python_vers}}.Functions.simulator(
	...
	{%- elif exec_target=="server" %}
	dblOutVal = SimulatorToFMU.Server.Functions.simulator(
	  modTim=time,
	  nDblInp=nDblInp,
	  dblInpNam=dblInpNam,
	  dblInpVal=dblInpVal,
	...
	{% for row in modelica_real_output_variable_names %}
	{{row}}_transfer.u = dblOutVal[{{loop.index}}];
	{%- endfor %}
	{% for row in modelica_real_output_variable_names %}
	connect({{row}}_transfer.y, {{row}});
	{%- endfor %}
  // End //
end {{model_name}};
